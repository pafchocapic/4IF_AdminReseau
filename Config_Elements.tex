...
\section{Spécification détaillée de l'architecture}
	...
	\subsubsection{Routeur VPN}
Ils sont à la base de l'architecture et sont situés à l'entrée de chacun des sites de l'AIP.
Ceux-ci permettent le communications sécurisées site à site, mais également les communications vers un PC distant.
...
\begin{itemize}
	...
	\item{VPN L2TP IPSEC} Ce mode de fonctionement ne propose pas d'outils de QoS ce qui empeche toute utilisation de ce mode pour la VoIP ou Visioconférence.
\end{itemize}

	\subsubsection{Switches Vlan}
Ces switches permettent d'isoler les équipements qui peuvent innonder le réseau. Ainsi, les plateformes de TP qui utilisent le Broadcast UDP peuvent être isolées dans un sous réseau, tout en restant accessible depuis l'extérieur.
	
	
	\subsubsection{Switches conventionnels}
	
	\subsubsection{Serveur DHCP}
	

	\subsubsection{Dimensionnement}
Pour le dimentionement de ces équipements
